...
def display_medication_info():
    global label_medication_name
    medication_data = load_medication_data()
    for row in medication_data:
        label_medication_name = tk.Label(top, text=row)
...
def add_medication():
    top1 = tk.Toplevel()
    top1.title("Medication Reminder Program")
    ...
    label_medication_name = tk.Label(top1, text="Medication Name:")
    label_medication_name.pack(pady=5)
    entry_medication_name = tk.Entry(top1)
    entry_medication_name.pack(pady=5)

    label_dosage = tk.Label(top1, text="Dosage:")
    label_dosage.pack(pady=5)
    entry_dosage = tk.Entry(top1)
    entry_dosage.pack(pady=5)

    label_frequency = tk.Label(top1, text="Frequency:")
    label_frequency.pack(pady=5)
    entry_frequency = tk.Entry(top1)
    entry_frequency.pack(pady=5)

    label_schedule_time = tk.Label(top1, text="Schedule Time:")
    label_schedule_time.pack(pady=5)
    entry_schedule_time = tk.Entry(top1)
    entry_schedule_time.pack(pady=5)

    button_add_medication = tk.Button(top1, text="Add Medication", command=add_medication_schedule)
    button_add_medication.pack(pady=5)
    top1.after(500, speaktext, "You had clicked on Add Medication App. Kindly enter your Medication, Dosage, Frequency and schedule time")
    top1.mainloop()
...
def display_medication():
    ...
    # Medication Schedule Form
    label_medication_name = tk.Label(top, text='Display Medication')
    label_medication_name.pack(pady=10)

    label_medication_info_btn = tk.Button(top, text='Click to view Medication', command=display_medication_info)
    label_medication_info_btn.pack(pady=10)
    
    top.after(500, speaktext, "You had clicked on Display Medication App successfully")
    top.mainloop()
...
label = tk.Label(window, text="Medication Reminder App", font=("Arial", 20), pady=20)
label.pack()
...
button3 = ttk.Button(button_frame, text="Exit/Close", command=close, style="Custom.TButton")
button3.pack(pady=10)

# Define custom styling for the buttons
#style = ttk.Style()
#style.configure("Custom.TButton", font=("Arial", 12), foreground="black", background="green", relief="raised")
#style.map("Custom.TButton", foreground=[('active', 'red'), ('disabled', 'gray')], background=[('active', 'pink'), ('disabled', 'gray')])

window.after(500, speaktext, "Welcome to our comprehensive Medication Reminder App offers features to add, display, set reminders, and exit with ease.")
